...
const TARGET = LYRICS.split("\n").map(line=>line.trim()).filter(Boolean).join("\n");
const TARGET_LINES = TARGET.split("\n");

type Msg = {type:string;name?:string;ready?:boolean;at?:number;typed?:number;correct?:number;finished?:number};
const code = () => Math.random().toString(36).slice(2,6).toUpperCase();
const accuracy = (text:string) => { let n=0; for(let i=0;i<text.length;i++) if(text[i]===TARGET[i]) n++; return n; };
...
export default function Home(){
  ...
  const mine=accuracy(typed); const progress=Math.round(typed.length/TARGET.length*100); const remoteProgress=Math.round(remote.typed/TARGET.length*100);
  ...
  {screen==="home"&&<section className="home"><div className="hero"><p>온라인 1대1 타자 대결</p><h1>떠강<br/>쨩쨩 대결</h1><span>방을 만들고 친구에게 코드를 알려주세요.<br/>두 컴퓨터에서 동시에 대결할 수 있어요.</span></div><div className="entry"><label>내 이름<input maxLength={12} value={name} onChange={e=>setName(e.target.value)} placeholder="이름을 입력하세요"/></label><button className="primary" onClick={createRoom}>새 방 만들기 <b>→</b></button><div className="or"><span/>또는<span/></div><label>방 코드<div className="join"><input maxLength={4} value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g,""))} placeholder="4자리 코드"/><button onClick={joinRoom}>참가하기</button></div></label><small>별도의 설치 없이 브라우저에서 바로 시작할 수 있습니다.</small></div></section>}
  ...
  {screen==="game"&&<section className="game"><div className="score"><Player name={name} value={progress} correct={mine} total={typed.length}/><div className="clock">{countdown?`0${countdown}`:"LIVE"}</div><Player name={other} value={remoteProgress} correct={remote.correct} total={remote.typed} reverse/></div><div className="lyricView"><span>{lineIndex+1} / {TARGET_LINES.length}번째 줄</span><p className="matchLine">{targetLine.split("").map((ch,i)=><b key={i} className={i>=lineInput.length?"waiting":lineInput[i]===ch?"correct":"wrong"}>{ch===" "?"\u00a0":ch}</b>)}</p><div className="nextLines">{nearbyLines.map((line,i)=><p key={i}>{line}</p>)}</div></div><input className="typeInput" ref={inputRef as React.RefObject<HTMLInputElement>} value={lineInput} onChange={e=>onType(e.target.value)} disabled={!playing} autoComplete="off" spellCheck={false} aria-label="현재 줄 입력" placeholder={countdown?`${countdown}초 후 시작합니다`:"위 문장을 입력하세요"}/><div className="gameFoot"><span>{typed.length.toLocaleString()} / {TARGET.length.toLocaleString()} 글자</span><span>정확도 {typed.length?Math.round(mine/typed.length*100):100}%</span></div>{winner&&<div className="result"><p>대결 종료</p><h2>{winner}</h2><strong>승리</strong><button onClick={()=>location.reload()}>새 게임</button></div>}</section>}
  <footer><span>떠강 쨩쨩 대결</span><span>두 참가자의 브라우저가 직접 연결됩니다.</span></footer></main>
}
